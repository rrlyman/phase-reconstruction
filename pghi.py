...
class PGHI(object):
    ...
    def __init__(self, a_a=128, M=2048, gl=None, g=None, tol = 1e-6, gamma = None, h = .01, plt=None, alg='p2015', pre_title='init', show_plots = False,  show_frames = 25):
        '''
        Parameters
            a_a    
                analysis hop size
        ...
        if gl is None: gl = M  
        if g is None:     
            gamma = (-np.pi*gl**2)/(4*np.log(h))
            lambda_ = (gamma/(2*np.pi))**.5
            g=np.array(signal.windows.gaussian(gl, lambda_, sym=False), dtype = dtype)

        self.a_a,self.M,self.tol,self.gamma,self.g,self.gl,self.h, self.pre_title  = a_a,M,tol,gamma,g,gl,h,pre_title
        ...
        if gamma is None: self.logprint('parameter error: must supply gamma and g')
...
#         dt =    (np.multiply(3,(xp[:-2,1:-1])    + np.multiply(2,xp[1:-1,1:-1])    + np.multiply(3,xp[2:,1:-1]))   - np.multiply(6,(xp[:-2,1:-1]    + xp[1:-1,1:-1]    + xp[2:,1:-1])))/6         
    ...
    def magnitude_to_phase_estimate(self, magnitude):  
        ...
        fmul = self.gamma/(self.a_a * self.M )
        
        tgradplus = (2*np.pi*self.a_a/self.M)*np.arange(self.M2) 
        tgrad = self.dxdw(logs)/fmul + tgradplus
        
        fgradplus = self.a_a/2          
...
#                 heapq.heappush(self.h, (-magnitude[N-1,m], N-1, m))                                     
        ...
        while np.any(self.active_mask): 
            ...
            # For debugging, adjust phase from the fft of the real input signal with this island peak
