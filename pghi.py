...
class PGHI(object):
    ...
    def __init__(self, a_a=128, M=2048, gl=None, g=None, tol = 1e-6, lambdasqr = None, gamma = None, h = .01, plt=None, alg='p2015', pre_title='', show_plots = False,  show_frames = 25, verbose=True, Fs=44100):
        ...
        self.plt.plot_waveforms("synthesis window", [self.gsynth])  
...
#         dt =    (np.multiply(3,(xp[:-2,1:-1])    + np.multiply(2,xp[1:-1,1:-1])    + np.multiply(3,xp[2:,1:-1]))   - np.multiply(6,(xp[:-2,1:-1]    + xp[1:-1,1:-1]    + xp[2:,1:-1])))/(*6 )        
    ...
    def magnitude_to_phase_estimate(self, magnitude):  
        ...
        startpoints = ndimage.maximum_position(magnitude*mask, labeled, range(nr_objects+1))[1:]     
...
#                 heapq.heappush(h, (-magnitude[N-1,m], N-1, m))   
        ...
        for startpoint in startpoints:
            ...
            n,m = startpoint
            self.phase[n,m] = 0
            heapq.heappush(h, (-magnitude[n,m],n,m)) 
            active_padded[n+1,m+1]=False
            if self.plt.verbose: 
                ...
                if self.original_phase is not None:
                    self.original_phase -= self.original_phase[n,m] 
            ...
            while len(h) > 0:
                ...
                if active_padded[(n+2), m+1]:  # East 
                    ...
                    self.phase[(n+1), m]=  self.phase[n,  m] + (tgrad[n,  m] + tgrad[(n+1), m])/2 
                    heapq.heappush(h, (-magnitude[(n+1), m], n+1,m))   
                    if self.plt.verbose and self.debug_count <= 2000 : 
                        self.debugInfo(n+1, m, n, m)                    
...
#                 if active_padded[n+1,m+2]: 
#                     self.integrate(n,m+1,n,m, 1, fgrad) # North                             
#                 if active_padded[n+1,m]: 
#                     self.integrate(n,m-1,n,m, -1, fgrad) # South                         
#                 if active_padded[n+2,m+1]: 
#                     self.integrate(n+1,m,n,m,1, tgrad)  # East                            
#                 if active_padded[n,m+1]: 
#                     self.integrate(n-1,m,n,m, -1, tgrad) # West                                               
#                 if active_padded[n+2,m+2]: self.integrate(n+1,m+1,n,m, 1, dphaseNE) # NE                          
#                 if active_padded[n+2,m]: self.integrate(n+1 ,m-1,n,m, 1, dphaseSE) #SE      
#                 if active_padded[n,m]: self.integrate(n-1,m-1,n,m, -1, dphaseNE) # SW    
#                 if active_padded[n,m+2]: self.integrate(n-1,m+1,n,m, -1, dphaseSE) # NE 
               
        if self.plt.verbose:
            if self.original_phase is not None: 
                self.plt.plot_3d('Phase original', [self.original_phase], mask=mask,startpoints=startpoints)          
                self.plt.plot_3d('Phase original, Phase estimated', [(self.original_phase) %(2*np.pi), ( self.phase) %(2*np.pi)], mask=mask, startpoints=startpoints)   
                self.plt.colorgram('Phase original minus Phase estimated', np.abs((self.original_phase) %(2*np.pi) -( self.phase) %(2*np.pi)), mask=mask, startpoints=startpoints)                      
                self.plt.quiver('phase errors (2pi)%', self.q_errors, startpoints= startpoints)               
            ...
            self.plt.plot_3d('Phase estimated', [self.phase], mask=mask,startpoints=startpoints)                         
            
        return self.phase
    
    def debugInfo(self, n1, m1, n0, m0):  
        ...
        if dif==0:
            ...
            err_new = (dif - dif_orig) 
        self.q_errors.append((n0,m0,0 ,n1-n0,m1-m0,err_new))                 
