...
def sweep_test():
    ...
    signal_in = signal.chirp(
        range(dur), freq_low / p.Fs, dur, freq_high / p.Fs, method=method
    )
    signal_in2 = signal.chirp(
        range(dur), freq_high / p.Fs, dur, freq_low / p.Fs, method=method
    )
...
def warble_test():
    ...
    dur = int(0.25 * p.Fs / samples_for_2_pi_radians)
    signal_in = []
    for k in range(dur):
        signal_in.append(
            signal.chirp(
